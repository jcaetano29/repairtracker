...
describe("getOrdenes — paginación", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  // Query builder encadenable y "thenable": await query resuelve a `result`.
  ...
  it("no pagina (no llama a range) cuando limit es 'all' — el kanban carga todo", async () => {
    const q = makeQuery({ data: [{ id: 1 }], count: 1, error: null });
    getSupabaseAdmin.mockReturnValue(q);

    const res = await dataModule.getOrdenes({ limit: "all" });

    expect(q.range).not.toHaveBeenCalled();
    expect(res.count).toBe(1);
  });

  it("pagina con range(from, to) cuando se pasa un limit numérico", async () => {
    const q = makeQuery({ data: [], count: 0, error: null });
    getSupabaseAdmin.mockReturnValue(q);

    await dataModule.getOrdenes({ page: 2, limit: 20 });
...
describe("getEmpleados", () => {
  ...
  it("returns empleados ordered by nombre", async () => {
    const mockData = [
      { id: "e1", sucursal_id: "s1", nombre: "Ana", created_at: "2026-01-01" },
    ];
    const mockClient = {
      from: vi.fn().mockReturnThis(),
      select: vi.fn().mockReturnThis(),
...
describe("crearOrden — empleado_id", () => {
  ...
  it("inserts empleado_id and a snapshot of the empleado's nombre when it belongs to the order's sucursal", async () => {
    const insertedOrden = { id: "o1", sucursal_id: "s1", empleado_id: "e1", empleado_nombre: "Ana" };
      ...
      from: vi.fn((table) => {
        if (table === "empleados") {
          return {
            select: vi.fn().mockReturnThis(),
            eq: vi.fn().mockReturnThis(),
            single: vi.fn().mockResolvedValue({ data: { sucursal_id: "s1", nombre: "Ana" }, error: null }),
          };
        }
        if (table === "ordenes") {
          return {
    ...
    expect(insertFn).toHaveBeenCalledWith(
      expect.objectContaining({ empleado_id: "e1", empleado_nombre: "Ana" })
    );
  });
});
