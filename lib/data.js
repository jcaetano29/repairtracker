...
export async function crearOrden({ cliente_id, tipo_articulo, marca, modelo, problema_reportado, notas_internas, monto_presupuesto, nombre_articulo, tipo_servicio_id }) {
...
export async function getReportesStats() {
  ...
  // Historical revenue (all delivered orders)
  const ingresosHistoricos = todasLasOrdenes
    .filter(o => o.estado === "ENTREGADO")
    .reduce((sum, o) => sum + (parseFloat(o.monto_presupuesto) || 0), 0);

  // Average ticket (historical, delivered orders with a budget)
  const entregadasConMonto = todasLasOrdenes.filter(o => o.estado === "ENTREGADO" && o.monto_presupuesto);
  const ticketPromedio = entregadasConMonto.length > 0
    ? Math.round(ingresosHistoricos / entregadasConMonto.length)
    : null;
