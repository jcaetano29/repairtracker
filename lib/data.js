...
import { TRANSICIONES } from "./constants";
import { normalizePhoneToE164 } from "./phone";

// Tables ordenes / v_ordenes_dashboard / clientes / historial_estados have
// admin-only RLS (see supabase/031_lockdown_pii.sql). Functions that touch
// those use getSupabaseAdmin() and MUST be called only from server-side
// (API routes, server components). Functions on open tables
// (talleres, sucursales, marcas, tipos_servicio) keep using the anon client
// because their RLS allows public read.
...
export async function getOrdenes({ estado, taller_id, busqueda, incluirEntregados = false, sucursal_id, page = 1, limit = 20 }) {
  ...
  if (busqueda) {
    ...
    if (sanitized.trim()) {
      const orFilters = [
        `cliente_nombre.ilike.%${sanitized}%`,
        `marca.ilike.%${sanitized}%`,
        `cliente_telefono.ilike.%${sanitized}%`,
      ];
      // numero_orden is an integer (SERIAL), so ilike doesn't apply.
      // Match it exactly when the term is numeric (handles "51" and "0051").
      // Guard against values above int4 range (e.g. a pasted phone number)
      // which would otherwise raise "integer out of range" on the whole query.
      const numeric = sanitized.replace(/\D/g, "");
      if (numeric && Number(numeric) <= 2147483647) {
        orFilters.push(`numero_orden.eq.${Number(numeric)}`);
      }
      query = query.or(orFilters.join(","));
    }
  }

  // limit === "all" desactiva la paginación: el kanban necesita todas las
  // órdenes activas, no solo la primera página (si no, el tablero muestra 20
  // mientras las tarjetas de stats cuentan el total).
  if (limit !== "all") {
...
export async function cambiarEstado(orden_id, nuevo_estado, extras = {}) {
  ...
  const { data: currentOrden } = await getSupabaseAdmin()
    .from("ordenes")
    .select("estado")
    .eq("id", orden_id)
    .single()
  ...
  if (currentOrden) {
    const allowed = TRANSICIONES[currentOrden.estado]
    if (allowed && !allowed.includes(nuevo_estado)) {
      throw new Error(
        `Transición no permitida: ${currentOrden.estado} → ${nuevo_estado}`
      )
    }
  }

  // LISTO_PARA_ENVIO requires taller_id
  if (nuevo_estado === "LISTO_PARA_ENVIO" && !extras.taller_id) {
    throw new Error("Se requiere asignar un taller para pasar a Enviar a Taller");
  }
...
export async function deleteOrden(orden_id) {
  ...
  const { error: historialError } = await getSupabaseAdmin().from("historial_estados").delete().eq("orden_id", orden_id);
  if (historialError) throw historialError;
  const { error } = await getSupabaseAdmin().from("ordenes").delete().eq("id", orden_id);
  if (error) throw error;
}

export async function cambiarTallerOrden(orden_id, taller_id) {
  const orden = await getOrden(orden_id);

  const estadosPermitidos = ["LISTO_PARA_ENVIO", "EN_TALLER", "EN_REPARACION"];
  if (!estadosPermitidos.includes(orden.estado)) {
    throw new Error("Solo se puede cambiar el taller mientras la orden está en camino o en el taller");
  }

  const { data: taller, error: tallerError } = await getSupabaseAdmin()
    .from("talleres")
    .select("id, activo")
    .eq("id", taller_id)
    .single();
  if (tallerError || !taller) throw new Error("Taller no encontrado");
  if (!taller.activo) throw new Error("El taller seleccionado no está activo");
...
export async function updateMarcaOrden(orden_id, marca) {
  const { data, error } = await getSupabaseAdmin()
    .from("ordenes")
    .update({ marca })
    .eq("id", orden_id)
...
export async function updatePresupuestoTaller(orden_id, monto_presupuesto_taller) {
  const { data, error } = await getSupabaseAdmin()
    .from("ordenes")
    .update({ monto_presupuesto_taller })
    .eq("id", orden_id)
    .select("*")
    .single();
...
export async function buscarClientes(query) {
  ...
  // El teléfono se guarda como dial+nacional sin el 0 de tronco (ej "59899123456"),
  // pero el usuario suele escribirlo con 0 (ej "099123456"). Normalizamos el término
  // al mismo formato guardado para que "099..." y "99..." matcheen igual.
  const e164 = normalizePhoneToE164(sanitized);
  if (e164) {
    const soloDigitos = e164.replace(/\D/g, ""); // "+59899123456" -> "59899123456"
    orFilters.push(`telefono.ilike.%${soloDigitos}%`);
    orFilters.push(`telefono_e164.ilike.%${e164}%`);
  }
  const { data, error } = await getSupabaseAdmin()
    .from("clientes")
    .select("*")
...
export async function listarTodosClientes() {
  const { data, error } = await getSupabaseAdmin()
    .from("clientes")
    .select("nombre, telefono, email, documento, created_at")
...
export async function crearCliente({ nombre, telefono, email, documento }) {
  const { data, error } = await getSupabaseAdmin()
    .from("clientes")
    .insert({ nombre, telefono, email, documento, telefono_e164: normalizePhoneToE164(telefono) })
    .select()
    .single();
...
export async function buscarClientePorTelefonoE164(telefonoE164) {
  const { data, error } = await getSupabaseAdmin()
    .from("clientes")
    .select("*")
...
// Error especial para conflictos de unicidad al editar un cliente. La API
// lo traduce a un 409 con el detalle del campo (documento/telefono).
export class ClienteDuplicadoError extends Error {
  constructor(campo) {
    super(campo === "documento" ? "Ya existe un cliente con este documento" : "Ya existe un cliente con este teléfono");
    this.name = "ClienteDuplicadoError";
    this.campo = campo;
  }
}

export async function actualizarCliente(id, { nombre, telefono, email, documento }) {
  const telefonoE164 = normalizePhoneToE164(telefono);

  // documento y telefono_e164 tienen índices únicos: chequeamos que el nuevo
  // valor no pertenezca a OTRO cliente antes de escribir.
  const { data: dupDoc, error: dupDocError } = await getSupabaseAdmin()
    .from("clientes")
    .select("id")
    .eq("documento", documento)
    .neq("id", id)
    .limit(1);
  if (dupDocError) throw dupDocError;
  if (dupDoc && dupDoc.length > 0) throw new ClienteDuplicadoError("documento");

  if (telefonoE164) {
    const { data: dupTel, error: dupTelError } = await getSupabaseAdmin()
      .from("clientes")
      .select("id")
      .eq("telefono_e164", telefonoE164)
      .neq("id", id)
      .limit(1);
    if (dupTelError) throw dupTelError;
    if (dupTel && dupTel.length > 0) throw new ClienteDuplicadoError("telefono");
  }
...
export async function getTalleresStats() {
  // v_talleres_stats runs as the caller (security_invoker) and reads ordenes
  // which is admin-only — must use the admin client.
  const { data, error } = await getSupabaseAdmin()
...
// Write operations on tipos_servicio require admin (only SELECT is open for
// anon). crearTipoServicio/updateTipoServicio/deleteTipoServicio are only
// called from /api/admin/tipos-servicio.
...
export async function getReportesStats(umbrales = {}, { sucursal_id = null } = {}) {
  const now = new Date();
  ...
  // Active orders with delay — caller passes umbrales (loaded from /api/configuracion).
  const { getNivelRetraso } = await import("./constants");
  ...
  const ordenesConRetraso = todasLasOrdenes.filter(o => {
    ...
    return getNivelRetraso(o.estado, dias, umbrales) !== "none";
  }).length;
...
// Write operations on talleres require admin (only SELECT is open for anon).
// crearTaller/updateTaller/deleteTaller are only called from /api/admin/talleres.
...
export async function deleteTaller(id) {
  // Desasociar órdenes de este taller antes de eliminar (ordenes is locked → admin)
  const { error: unlinkError } = await getSupabaseAdmin()
    .from("ordenes")
    .update({ taller_id: null })
    .eq("taller_id", id);
  if (unlinkError) throw unlinkError;

  const { error } = await getSupabaseAdmin().from("talleres").delete().eq("id", id);
  if (error) throw error;
}
...
// Write operations on marcas require admin (only SELECT is open for anon).
// crearMarca/updateMarca/deleteMarca are only called from /api/admin/marcas.
