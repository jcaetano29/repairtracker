...
export async function crearOrden({ cliente_id, tipo_articulo, marca, modelo, problema_reportado, notas_internas, monto_presupuesto, moneda, nombre_articulo, tipo_servicio_id, sucursal_id, material, material_otro, peso_gramos, monto_presupuesto_taller }) {
  ...
  try {
    ...
    if (!esCentro && centros.length > 0) {
      ...
      if (!existing) {
        // Default to first repair center
        await crearTraslado({
          orden_id: orden.id,
