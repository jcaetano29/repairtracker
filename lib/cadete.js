...
// Server-only: all functions touch tables with admin-only or mixed RLS
// (resumenes_cadete, items_resumen_cadete, usuarios). The file is imported
// solely by API routes in app/api/resumenes-cadete/ and app/api/cadete/.
...
export async function addItemOrden(resumen_id, orden_id, subtipo) {
  const { data: lastItem, error: orderErr } = await getSupabaseClient()
    .from("items_resumen_cadete")
    .select("orden")
    .eq("resumen_id", resumen_id)
    .order("orden", { ascending: false })
