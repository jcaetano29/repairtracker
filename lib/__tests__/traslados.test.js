...
describe("traslados data layer", () => {
  ...
  describe("despacharTraslado", () => {
    it("should update traslado to en_transito with fecha_salida", async () => {
      const now = new Date();
      const mockTraslado = {
        id: "traslado-1",
        estado: "en_transito",
      ...
      expect(mockClient.update).toHaveBeenCalledWith(
        expect.objectContaining({
          estado: "en_transito",
          fecha_salida: expect.any(String),
        })
      );
      expect(mockClient.eq).toHaveBeenCalledWith("id", "traslado-1");
    });
  });
  ...
  describe("recibirTraslado", () => {
    it("should update traslado to recibido and update orden sucursal_id", async () => {
      const now = new Date();
      const traslado = {
        id: "traslado-1",
        orden_id: "orden-1",
      ...
      expect(result).toEqual(traslado);
      expect(mockClient.from).toHaveBeenCalledWith("traslados");
  ...
  // ===================================================================
  // getCentroReparacion Tests
  // ===================================================================

  describe("getCentroReparacion", () => {
    it("should get the repair center sucursal", async () => {
      const mockCentro = {
        id: "centro-1",
    ...
    it("should throw error if no repair center found", async () => {
      const error = new Error("No repair center found");
      const mockClient = {
        from: vi.fn().mockReturnThis(),
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
      ...
      await expect(trasladosModule.getCentroReparacion()).rejects.toThrow(
        "No repair center found"
      );
    });
  });
