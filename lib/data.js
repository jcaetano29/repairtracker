...
export async function crearOrden({ cliente_id, tipo_articulo, marca, modelo, problema_reportado, notas_internas, monto_presupuesto, moneda, nombre_articulo, tipo_servicio_id, sucursal_id }) {
  ...
  // Auto-create ida transfer if sucursal is not the repair center
  try {
    const { getCentroReparacion, crearTraslado, getTrasladoActivo } = await import("./traslados");
...
export async function cambiarEstado(orden_id, nuevo_estado, extras = {}) {
  ...
  try {
    const { getTrasladoActivo } = await import("./traslados");
    ...
    if (e.message.includes("traslado")) throw e;
    // If traslados module fails, proceed (backwards compatibility)
  }
...
export async function deleteOrden(orden_id) {
  ...
  try {
    ...
    if (e.message.includes("traslado activo")) throw e;
    // If traslados module fails, proceed with delete (backwards compatibility)
  }
...
export async function updateSucursalRetiro(orden_id, sucursal_retiro_id) {
  ...
  if (orden.estado === "LISTO_PARA_RETIRO") {
    try {
      const { getTrasladoActivo, crearTraslado } = await import("./traslados");
