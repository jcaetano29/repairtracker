...
function renderTicket(doc, { orden, cliente, nombreNegocio, copia, showNotas, showCliente, showEmpleado, pageWidth, margin, contentWidth }) {
  let y = 6
  ...
  doc.setFontSize(8)
  doc.setFont("helvetica", "normal")
  const fechaIngreso = new Date(orden.fecha_ingreso || Date.now()).toLocaleString("es-UY", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  // --- Empleado (store copy only, never on the client copy) ---
  if (showEmpleado && orden.empleado_nombre) {
    doc.setFont("helvetica", "bold")
    doc.text("EMPLEADO", margin, y)
    y += 4
    doc.setFont("helvetica", "normal")
    doc.text(orden.empleado_nombre, margin, y)
    y += 4 + 2
  }
  ...
  if (orden.fecha_entrega_estimada) {
    const fechaEntrega = new Date(orden.fecha_entrega_estimada + "T12:00:00").toLocaleDateString("es-UY", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
    })
  ...
  doc.setFont("helvetica", "normal")
  if (orden.en_garantia) {
    doc.text("En garantia - Sin cargo", margin, y)
  } else if (orden.monto_presupuesto) {
    doc.text(formatMonto(orden.monto_presupuesto, orden.moneda), margin, y)
  } else {
    doc.text("A confirmar", margin, y)
  }
  y += 5
