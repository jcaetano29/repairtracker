...
export async function getTraslados({ sucursal_id } = {}) {
  let query = getSupabaseClient()
    .from("traslados")
    .select(
      "*, ordenes!inner(id, numero_orden, tipo_articulo, marca, cliente_id, clientes!inner(id, nombre, telefono))"
    )
    .neq("estado", "recibido");
...
export async function recibirTraslado(traslado_id, recibido_por) {
  ...
  // First, get the traslado to know the destino
  ...
  // Update the traslado
  const { data: updatedTraslado, error: updateError } = await supabase
    .from("traslados")
    .update({
      estado: "recibido",
      recibido_por,
  ...
  // Update the order's sucursal_id to the traslado's destination
...
/**
 * Get the sucursal where es_centro_reparacion=true.
 *
 * @returns {Promise<Object>} The repair center sucursal record
 */
