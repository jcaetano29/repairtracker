-- supabase/040_snapshot_nombre_empleado.sql
-- Guarda el nombre del empleado como texto en la propia orden (captura fija
-- al momento de crearla), además de la referencia por empleado_id. Así,
-- si el empleado se borra más adelante, la orden sigue mostrando quién la
-- ingresó — ya no depende de que el registro en `empleados` siga existiendo.

BEGIN;

ALTER TABLE ordenes ADD COLUMN empleado_nombre TEXT;

-- Backfill: para órdenes que ya tienen empleado_id pero todavía no tienen
-- el nombre capturado, lo tomamos de `empleados` mientras el registro exista.
UPDATE ordenes o
SET empleado_nombre = e.nombre
FROM empleados e
WHERE o.empleado_id = e.id AND o.empleado_nombre IS NULL;

-- Redefinir v_ordenes_dashboard: empleado_nombre ahora sale directo de
-- ordenes (la captura fija), no del JOIN a empleados — ya no se pierde si
-- el empleado se borra.
