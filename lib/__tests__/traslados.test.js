...
// Most traslados functions use the admin client (server-only). Only
// getCentrosReparacion uses the anon client because it reads the open
// 'sucursales' table from the browser.
...
import { getSupabaseClient as getSupabaseAnon } from "../supabase-client";
// Backwards-compat alias: existing assertions use getSupabaseClient.mockReturnValue
// for the admin client (most tests). getCentrosReparacion tests below override
// the anon mock directly.
const getSupabaseClient = getSupabaseAdmin;
...
describe("traslados data layer", () => {
  ...
  describe("getCentrosReparacion", () => {
    it("should get all repair center sucursales", async () => {
      ...
      getSupabaseAnon.mockReturnValue(mockClient);
    ...
    // (second test sets up its own anon mock below)
